...
import { faChartBar, faComment, faList, faPlus, faSpinner,
  faSquareRootAlt, faSync, faTags, faTasks } from '@fortawesome/free-solid-svg-icons';
...
export class MainComponent implements OnInit, OnDestroy {
  ...
  private emitFinishedStep = new EventEmitter();

  private tags: Tag[];
  private transformNameDict: { [name: string]: string } = {};
  ...
  private _isCacheReady(): boolean {
    return !!this.tags && !!this.transformNameDict;
  }
  ngOnInit() {
  ...
  private _subscribeToFinishedStepEvents() {
    this.emitFinishedStep.subscribe((step: FinishedStep) => {
  ...
  private _decklistPostProcessing() {
    ...
    this.emitFinishedStep.emit(FinishedStep.PostProcessing);
  }
    ...
    const sub = this.notify.isTagsUpdated$.subscribe((event: iTagsUpdated) => {
      switch (event.type) {
        case EventType.Insert: {
          ...
          if (event.Tag) {
            let tagIndex = 0;
            while (tagIndex < this.tags.length) {
              if (event.Tag.name > this.tags[tagIndex].name) {
                tagIndex++;
              } else {
                break;
              }
            ...
            this.tags.splice(tagIndex, 0, event.Tag);
          }
          break;
        }
        ...
        case EventType.Merge: {
          // update list of tags
          if (event.Tag) {
            const fromIndex = this.tags.findIndex(m => m.id === event.fromId);
            if (fromIndex !== -1) {
              this.tags.splice(fromIndex, 1);
            }
  ...
  private _submitDecklistHelper(lines: string[]) {
    ...
    if (lines.length === 0) {
      this.emitFinishedStep.emit(FinishedStep.Oracle);
    } else {
      /*
          ...
          this.oracle.getTransform()
            .subscribe(cards => {
              if (cards) {
                ...
                this.emitFinishedStep.emit(FinishedStep.Cache);
              }
            });
        });
    }
  ...
  private _mixinTagLinksHelper(oracle_ids: string[]) {
    if (oracle_ids.length === 0) {
      this.emitFinishedStep.emit(FinishedStep.CardTagLinks);
    } else {
